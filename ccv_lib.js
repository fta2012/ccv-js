var Module = {
  ...
  readImageData: function(source) {
    ...
    if (Module.isImageData(source)) {
      ...
      if (!element) {
        console.error("Can't convert", source, 'to element');
        return;
      }
      var canvas;
      var context;
      if (element instanceof HTMLCanvasElement) {
        ...
        // Image or Video
        var width = element.videoWidth || element.width;
        var height = element.videoHeight || element.height;
  ...
  writeImageData: function(dest, data, width, height) {
    console.assert(dest);
